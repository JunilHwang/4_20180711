...
class Model {
	...
	function init () {
		$this->db = new PDO("mysql:host=127.0.0.1;dbname=20180711;charset=utf8","root","xampp");
	...
	function query ($sql = false) {
		...
		if (!$res->execute($this->execArr)){
			print_pre(['sql'=>$this->sql], false);
			print_pre(['execArr'=>$this->execArr], false);
			print_pre(['db'=>$this->db->errorInfo()]);
		}
